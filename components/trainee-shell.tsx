...
import { CourseReviewGate } from "@/components/course-review-gate";
import { NotificationBell } from "@/components/notification-bell";
import { PageBreadcrumb } from "@/components/page-breadcrumb";
import { SignOutButton } from "@/components/sign-out-button";
import { UsageTracker } from "@/components/usage-tracker";
import { UserAvatar, formatRoleLabel } from "@/components/user-avatar";
import { searchTraineeWorkspace } from "@/lib/api/trainee";
import { ApiClientError } from "@/lib/api/client";
import { useAuth } from "@/providers/auth-provider";
import type { AuthUser } from "@/types/api";
import type { TraineeSearchHit, TraineeSearchResults } from "@/types/trainee-dashboard";
...
const TRAINEE_NAV: NavItem[] = [
  ...
  { href: "/trainee/progress", label: "Your Journey", icon: GraduationCap },
  { href: "/trainee/announcements", label: "Announcements", icon: Megaphone },
  { href: "/trainee/leaderboard", label: "Leaderboard", icon: Trophy },
  { href: "/trainee/achievements", label: "Achievements", icon: Award },
  { href: "/trainee/requirements", label: "Requirements", icon: ClipboardList },
  { href: "/trainee/certificates", label: "Certificates", icon: Award },
  { href: "/trainee/profile", label: "Profile", icon: User },
];

type TraineeShellProps = {
  title: string;
  user: AuthUser;
  actions?: ReactNode;
  dense?: boolean;
...
const EMPTY_SEARCH: TraineeSearchResults = {
  programs: [],
  materials: [],
  assignments: [],
  assessments: [],
...
export function TraineeShell({
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      setCollapsed(sessionStorage.getItem("lms-trainee-sidebar") === "1");
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem("lms-trainee-sidebar", collapsed ? "1" : "0");
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <header className="sticky top-0 z-20 flex shrink-0 items-center gap-3 border-b border-slate-200/70 bg-white/90 px-4 py-3 backdrop-blur-md md:px-6">
          <div className="flex items-center gap-2">
            <Link href="/trainee" className="flex items-center gap-2 rounded-xl pr-1">
          ...
          <NotificationBell role="trainee" allHref="/trainee/announcements" />
        ...
        {!hideHeader && dense ? (
          <div className="shrink-0 border-b border-slate-200/70 bg-white px-4 py-3 md:px-8">
        ...
        <main className={dense ? "flex min-h-0 flex-1 flex-col overflow-y-auto" : "min-h-0 flex-1 overflow-y-auto px-4 py-6 md:px-8 md:py-8"}>
          {!hideHeader && !dense ? (
            <div className="mb-6">
              <PageBreadcrumb entityLabel={crumbLabel} />
              <div className="flex flex-wrap items-end justify-between gap-3">
...
type FlatHit = TraineeSearchHit & { group: string };

function flattenResults(results: TraineeSearchResults): FlatHit[] {
  return [
    ...results.programs.map((item) => ({ ...item, group: "Courses" })),
    ...results.materials.map((item) => ({ ...item, group: "Materials" })),
    ...results.assignments.map((item) => ({ ...item, group: "Assignments" })),
    ...results.assessments.map((item) => ({ ...item, group: "Quizzes" })),
    ...results.announcements.map((item) => ({ ...item, group: "Announcements" })),
  ];
}
...
function TraineeSearch() {
  ...
  const [results, setResults] = useState<TraineeSearchResults>(EMPTY_SEARCH);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      searchTraineeWorkspace(trimmed)
        .then((payload) => {
  ...
  const grouped = [
    { label: "Courses", items: results.programs },
    { label: "Lessons", items: results.materials },
    { label: "Assignments", items: results.assignments },
    { label: "Quizzes", items: results.assessments },
    { label: "Announcements", items: results.announcements },
  ...
  return (
    <div ref={rootRef} className="relative mx-auto w-full max-w-xl">
      ...
      {open && query.trim().length >= 2 ? (
        <div id={listId} role="listbox" className="absolute inset-x-0 top-[calc(100%+8px)] z-30 max-h-80 overflow-auto rounded-2xl bg-white py-2 shadow-xl ring-1 ring-slate-950/5">
          ...
          {!loading && !error && hits.length === 0 ? <p className="px-4 py-3 text-sm text-slate-500">No matches in your learning.</p> : null}
          {!loading && !error
            ? grouped.map((group) => (
