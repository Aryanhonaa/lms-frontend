...
function QuizPreview({ quiz }: { quiz: Quiz }) {
  return (
    <div className="border border-stone-200 bg-white px-4 py-3 text-sm">
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      ...
      {program.weeks.map((week, weekIndex) => (
        <section key={week.id} className="bg-white px-5 py-5">
          <h3 className="font-medium text-stone-950">
            Week {weekIndex + 1}: {week.title}
          </h3>
          {week.description ? <p className="mt-1 text-sm text-stone-600">{week.description}</p> : null}
      ...
      {program.milestones.map((milestone) => (
        <section key={milestone.id} className="bg-white px-5 py-5">
          <h3 className="font-medium text-stone-950">Milestone · {milestone.title}</h3>
