...
import {
  ...
  currentDayItems,
  doThisFirst,
  exhaustedQuizContinueCopy,
  flattenLearnPath,
  friendlyLockReason,
  isActionableStatus,
  isDoneStatus,
  isQuizProgressionLock,
  locationLabel,
  lockCopyForItem,
  nextUnlockItem,
  pathItemStatusCopy,
...
export function LearnWorkspace({ view, batchId, currentType, currentId, onViewChange, toolbar }: LearnWorkspaceProps) {
  ...
  const current =
    ...
    path.find((item) => isActionableStatus(item.status, item.canRetry)) ??
  ...
  const today = currentDayItems(view, path, current);
  ...
  const currentStatus = current ? pathItemStatusCopy(current, true) : null;
  const lockMessage = current ? lockCopyForItem(current, path) : "";
  const exhaustedGate = Boolean(
    current &&
      current.status === "LOCKED" &&
      isQuizProgressionLock(current.reason) &&
      lockMessage === exhaustedQuizContinueCopy(),
  );
    ...
    <div className="relative flex min-h-0 flex-1 flex-col bg-[#f6f7fb] md:flex-row">
      ...
      <div className="border-b border-slate-200/80 bg-white px-4 py-3 md:hidden">
        <button type="button" className={traineeSecondaryCtaClass} onClick={() => setSidebarOpen((open) => !open)}>
          {sidebarOpen ? "Hide course" : "Course roadmap"}
        </button>
      </div>
        ...
        {current ? (
          <div className="mx-auto max-w-3xl space-y-5">
            <div className="lms-fade-up">
              <p className="text-sm font-semibold text-slate-900">{view.program.title}</p>
              <p className="mt-1 text-sm text-slate-500">{locationLabel(current.weekTitle, current.dayTitle)}</p>
              ...
              {todayStats.total > 0 ? (
                <p className="mt-2 text-sm font-medium text-slate-600">
                  ...
                  <span className="font-normal text-slate-400"> · this day</span>
                </p>
              ) : null}
            </div>
            ...
            <div className="lms-fade-up">
              {current.status === "LOCKED" ? (
                <LockCard
                  reason={lockMessage}
                  exhaustedAttempts={exhaustedGate}
                  doFirst={!exhaustedGate && firstAction && firstAction.id !== current.id ? firstAction : null}
                  doFirstHref={
                    !exhaustedGate && firstAction && firstAction.id !== current.id
                      ? traineeContinueHref(view.program.id, firstAction, batchId)
                      : undefined
                  }
                  unlocksTitle={exhaustedGate ? null : current.title}
                />
            ...
            {!next && current.status === "FAILED" && !current.canRetry ? (
              <p className="text-sm text-slate-500">
                You have used all available attempts. You can continue with the course. This assessment was not passed.
              </p>
            ) : !next && isDoneStatus(current.status) && current.kind === "FINAL_EXAM" ? (
              <p className="text-sm text-slate-500">You have finished this course.</p>
