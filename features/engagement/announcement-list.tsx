...
export function AnnouncementList({
  ...
  if (announcements.length === 0) {
    return <p className="px-5 py-6 text-sm text-stone-500">{empty}</p>;
  }
      ...
      {announcements.map((item) => (
        <li key={item.id} className="px-5 py-4">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            ...
            <p className="text-xs uppercase tracking-wide text-stone-500">{audienceLabel(item)}</p>
          </div>
          ...
          {item.audience === "TRAINEES_SELECTED" && item.recipients.length > 0 ? (
            <p className="mt-2 text-xs text-stone-500">To {item.recipients.map((person) => person.name).join(", ")}</p>
          ) : null}
